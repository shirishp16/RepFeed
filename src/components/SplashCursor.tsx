...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(cvs: HTMLCanvasElement) {
      ...
      let g: any = cvs.getContext('webgl2', params);
      const isWebGL2 = !!g;
    ...
    function supportRenderTextureFormat(g: any, internalFormat: number, format: number, type: number) {
    ...
    class Material {
      ...
      constructor(vs: any, fs: string) { this.vertexShader = vs; this.fragmentShaderSource = fs; this.programs = []; this.activeProgram = null; this.uniforms = []; }
      setKeywords(keywords: string[]) {
        let hash = 0;
        for (let i = 0; i < keywords.length; i++) hash += hashCode(keywords[i]);
        let program = this.programs[hash];
        if (program == null) { const fs = compileShader(gl.FRAGMENT_SHADER, this.fragmentShaderSource, keywords); program = createProgram(this.vertexShader, fs); this.programs[hash] = program; }
    ...
    class GLProgram {
      ...
      constructor(vs: any, fs: any) { this.uniforms = {}; this.program = createProgram(vs, fs); this.uniforms = getUniforms(this.program); }
    ...
    function getUniforms(program: any) {
      const u: any = []; const c = gl.getProgramParameter(program, gl.ACTIVE_UNIFORMS);
      for (let i = 0; i < c; i++) { const n = gl.getActiveUniform(program, i).name; u[n] = gl.getUniformLocation(program, n); }
      return u;
    }

    function compileShader(type: number, source: string, keywords?: string[]) {
      source = addKeywords(source, keywords); const s = gl.createShader(type); gl.shaderSource(s, source); gl.compileShader(s);
    ...
    const displayShaderSource = `
      precision highp float; precision highp sampler2D; varying vec2 vUv; varying vec2 vL; varying vec2 vR; varying vec2 vT; varying vec2 vB;
    ...
    const advectionShader = compileShader(gl.FRAGMENT_SHADER, `
      precision highp float; precision highp sampler2D; varying vec2 vUv; uniform sampler2D uVelocity; uniform sampler2D uSource;
    ...
    const pressureShader = compileShader(gl.FRAGMENT_SHADER, `
      precision mediump float; precision mediump sampler2D; varying highp vec2 vUv; varying highp vec2 vL; varying highp vec2 vR; varying highp vec2 vT; varying highp vec2 vB;
    ...
    function applyInputs() { pointers.forEach((p: any) => { if (p.moved) { p.moved = false; splatPointer(p); } }); }
    ...
    // Teal-tinted fluid colors matching RecoverFeed theme
    function generateColor() {
      const c = HSVtoRGB(Math.random() * 0.15 + 0.45, 0.7, 0.15); // teal-cyan range, dark
      return c;
    }
  ...
  return (
    <div className="fixed inset-0 z-[81] pointer-events-none w-full h-full">
      <canvas
        ref={canvasRef}
